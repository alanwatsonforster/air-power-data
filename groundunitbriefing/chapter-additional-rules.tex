...
\rulesection{Anti-Aircraft Artillery}

\rulesubsection{Laser Rangefinders (LRFs)}
If an AAA unit has an FCR-R and an LRF, then it does not suffer a \plus{1} modifier if the FCR is jammed or shut down.
If an AAA unit has an FCR-W and an LRF, then it only suffers a \plus{1} modifier (not a \plus{2} modifier) if the FCR is jammed or shut down.
