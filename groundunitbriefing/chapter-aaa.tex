...
\note{AAA Values}{
    ...
    \begin{itemize}
        ...
        \item ZPU-2: {\MP} gives this a damage rating of 3 (more than the ZPU-4) whereas AS gives it a rating of 1 (equal to the ZPU-1). Pipes gives this hit rolls of 4/3/1 whereas AS gives it 4/3/2. The ZPU-2 should have both hit rolls and damage ratings intermediate to those of the ZPU-1 and ZPU-4, but this is impossible as these only differ by one. The solution in {\AirStr} seems to be a good compromise: give it the hit rolls of the ZPU-4 but the damage rating of the ZPU-1. I adopt the AS values.
    ...
    \begin{itemize}
        ...
        \item Mobile ZPU-4: The original hit rolls for the mobile ZPU-4 section are 3/3/2, but 4/3/2 is predicted from the values of 5/4/3 for the battery and is in better agreement with the values for the M16. {\MP} gives the section the same hit rolls as the battery, despite the fewer mounts, which does not seem to be correct.

        \item M163: {\MP} gives the section the same hit rolls as the battery, despite the fewer mounts, which does not seem to be correct. I adopt the {\AirStr} hit rolls, which are in agreement with the empirical model.

        \item Panhard M3 VDA: The Panhard M3 VDA in {\AirStr} has unexpectedly low original hit rolls of 2/2/1 compared to the TCM-20 battery and the M3 TCM-20 section, despite having very similar guns. The values of 3/3/2 given by {\MP} are in agreement with those of the M3 TCM-20 section and those predicted from the TCM-20 battery.
...
\begin{withcounters}
    ...
    \note{M2 Platoon}{
        ...
        The AAA values are from {\AirStr} and {\MP}.
    ...
    \note{M16 Section}{
        ...
        The AAA values are from {\EOTG} and {\MP}.
    ...
    \note{M55 Battery}{
        ...
        The AAA values are the same as the M16.
    ...
    \note{ZPU-1/2/4 Battery}{
        ...
        The AAA values for the ZPU-1 are from {\EOTG}, {\TSOH}, and {\MP}.
        The AAA values for the ZPU-2 are from {\AirStr} rather than {\MP}, for the reasons given in the extended note above.
        The AAA values for the ZPU-4 are from {\AirStr} and {\MP} rather than {\TSOH}, for the reasons given in the extended note above.
    ...
    \note{BTR-152A Section}{
        ...
        Like the BTR-40, the BTR-152 is based on a truck and does not seem to have especially good mobility; we might consider giving it “G/P” mobility rather than “U”.
    ...
    \note{M167 Platoon}{
        ...
        The 1976 Operator's Manual describes the use of the AN/TVS-2B or AN/TVS-5 night sight. These are first- and second-generation image intensifiers. They are mounted next to the tracking sight and appear to be useful only for ground targets.
    ...
    \note{M163 Section}{
        ...
        The AAA values are from {\AirStr}. I do not use the hit rolls from {\MP} for the reasons given in the extended note above.

        The 1969 report and 1976 manual mention the AN/VPS-2 range-only radar. MobileRadar states it operates at 9.2--9.5 GHz.

        The 1976 Operator's Manual describes the use of the AN/TVS-2B night sight. This is a first-generation image intensifier. It is mounted next to the tracking sight and appears to be useful only for ground targets.
    ...
    \note{Rh-202 Battery}{
        ...
        Single Rh-202 \input{aaavalues/single Rh-202 battery.tex}
        Twin Rh-202 \input{aaavalues/twin Rh-202 battery.tex}

        The AAA values are from {\AirStr}, with the hit values for the single mount changed from 3/2/1 to 4/3/2 and the damage value for the twin mount changed from 2 to 3, for the reasons given in the extended note above. I do not use the values from {\MP} for the reasons given in the extended note above.

        {\AirStr} has single and twin Rh-202 batteries.
        Single mounts seem to be used for AFVs (e.g., Marder, Luchs, and Wiesel) and naval applications, but all AAA mounts seem to be twin.
    ...
    \note{ZU-23 Battery}{
        ...
        The AAA values are from {\AirStr}, {\TSOH}, and {\MP}, but the hit rolls are modified as from 4/3/2 to 5/3/2 and the damage rating from 2 to 3 as described in the extended note above.
    ...
    \note{Mobile ZU-23 Section}{
        ...
        The AAA values are from {\AirStr}, {\TSOH}, and {\MP}, but the hit rolls of 4/3/2 were obtained from those for the ZU-23 battery using the recipe described above, and the damage was modified from 2 to 3 as described in the extended note above.
    ...
    \note{Sinai 23 Section}{
        ...
        The AAA properties are based on the mobile ZU-23, but with \plus{1} to the hit rolls for the FCR-R giving 5/4/3. This means that the Sinai 23 (with two barrels) then has the same hit rolls and damage rating as the ZSU-23-4 (with four barrels). Perhaps this is acceptable, since there are 30 years between these systems.
    ...
    \note{M53/59 Section}{
        ...
        I adopted the AAA values from {\AirStr} and {\MP}, but reduced the hit rolls from 4/3/2 to 3/2/1 to be consistent with the AMX-30SA and Oerlikon GDF, as described in the extended note above.
    ...
    \note{AMX-13DCA Section}{

        \input{aaavalues/AMX-13DCA section.tex}

        I adopt the {\AirStr} values given for the AMX-30 DCA.

        {\MP} gives slightly different values for different 30 mm systems. For the Tunguska and M53/59, he gives a maximum altitude of 11 and ranges of 4/7/9. For the AMX-13, BOV 30, and K-30 he gives 10 and 3/6/9. However, there seems to be very little difference between the muzzle velocities and masses of the shells fired by these guns. I prefer to keep the {\AirStr} values for the AMX-30 DCA, which match those he gives for the Tunguska and M53/59.

        {\MP} also gives a damage rating of 4. This is the same as the Tunguska, despite the latter having a much higher rate of fire. I prefer to use the {\AirStr} value for the AMX-30 DCA of 3.
